...
// Default replacer using env vars
const defaultBaseUrlI = (() => {
  const i = import.meta.env.VITE_PXIMG_BASEURL_I
  return i ? (i.endsWith('/') ? i : i + '/') : PXIMG_I
})()
const defaultBaseUrlS = (() => {
  const s = import.meta.env.VITE_PXIMG_BASEURL_S
  return s ? (s.endsWith('/') ? s : s + '/') : PXIMG_S
})()

const defaultReplacer = createPximgReplacer(defaultBaseUrlI, defaultBaseUrlS)
export const replacePximgUrl = defaultReplacer.replacePximgUrl
export const replacePximgInObject = defaultReplacer.replacePximgInObject
