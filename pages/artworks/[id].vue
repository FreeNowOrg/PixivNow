...
async function init(id: string): Promise<void> {
  ...
  const dataCache = siteCache.get(`illust.${id}`)
  const pageCache = siteCache.get(`illust.${id}.page`)
...
async function handleUserInit(userId: string): Promise<void> {
  const value = siteCache.get(`user.${userId}`)
  if (value) {
    user.value = value
    return
  }
...
effect(() => useHead({ title: `${illust.value?.illustTitle} | Artworks` }))
