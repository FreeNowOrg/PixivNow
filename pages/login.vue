...
NForm#login-form.not-logged-in(v-if='!userStore.isLoggedIn')
  ...
  .tips
    ...
    p
      | 它应该形如：
      code(@click='getExampleSessionId' title='此处的令牌为随机生成，仅供演示使用') {{ example }}
      | 。
...
#login-form.logged-in(v-if='userStore.isLoggedIn')
  ...
  h1 已登录
  #submit
    NButton(@click='async () => await remove()' type='error') 移除令牌
</template>
...
async function submit(): Promise<void> {
  ...
  try {
    loading.value = true
    await userStore.login(sessionIdInput.value)
    error.value = ''
    goBack()
  } catch (err) {
