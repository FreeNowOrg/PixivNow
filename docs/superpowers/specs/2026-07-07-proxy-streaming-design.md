...
// - 从 upstream.headers 按 responseHeaders 白名单挑出，构造 new Headers
// - PROD：return new Response(upstream.body, { status: upstream.status, headers })
//   （白名单-only；body 为 ReadableStream 时 h3 sendStream 流式；upstream.body 为 null 时
//    sendWebResponse 走 res.end()，天然正确处理 HEAD / 204 / 304）
// - DEV 且有 body：upstream.body.tee() → 一支进 new Response 返回，
//   另一支封顶累积（默认 8KB）并打印 `[devLabel status] <preview>`（预览取前 200 字符）
// 注：不使用 setResponseHeader/setResponseStatus；返回的 Response 经 h3 sendWebResponse 驱动全部头+状态。
...
5. **content-encoding 已核实无坑**:undici 自动解压后 `Response.headers` 仍带 content-encoding;两个白名单均不转发它,故不会客户端二次解压。这也是 API **必须**白名单-only、不能 raw Response 透传的原因(现状 API 返回对象、图片因 i.pximg 不 gzip 才一直没炸)。
6. **图片响应头收紧为白名单-only(已确认)**:现状图片反代 `return response` 经 h3 `sendWebResponse` 实际转发**全部**上游头;改后仅转发白名单(accept-ranges/content-length/content-range/content-type/etag/cache-control/expires/last-modified/content-disposition),丢弃 date/age/vary/cf-* 等非必要头。回归到原 `setResponseHeader` 白名单的本意。用户已确认接受此收紧。

补充说明(请求头不对称,有意保留):API 反代对 pixiv.net **全转请求头 + 覆写 origin/referer/UA/cookie**(cookie/PHPSESSID 是登录态载体,必须发);图片反代对 i.pximg 用**请求头白名单**(不含 cookie/authorization,避免把用户凭据泄露给 CDN)+ 注入 referer/UA(i.pximg 校验 referer)。本次重构两者请求头逻辑均**原样不动**。
